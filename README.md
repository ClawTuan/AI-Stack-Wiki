...
为了平衡高频录入和全局架构维护，本仓库采用 **Trae CN 版** 与 **Trae 国际版** 同机协同的工作流：

- **Trae CN 版（高频产出者）**：负责高频的材料解析和录入任务（如解析论文、抽取知识点、生成 `wiki/concepts/`、`wiki/models/` 等笔记）。借助本地软链接，CN 版可直接读取系统外的 `raw` 共享目录，且全程使用纯 Python 的 `pypdf` 库进行文本抽取，**零系统依赖**。
- **Trae 国际版（架构维护者）**：负责低频但关键的全局工作（如 `SCHEMA.md` 规范调整、`README.md` 维护、目录树重构以及整体结构校验）。

两版 Trae 各司其职，通过 Git 分支和 PR（如 `cn/ingest-*` → `main`）实现解耦与协同。
...
录入新材料并生成/更新笔记后，本仓库支持自动生成 Markdown 原生的知识图谱（基于 Mermaid）。它会解析 `wiki` 目录下所有 Markdown 文件之间的相对链接，自动抽取出实体节点和关联关系，并渲染到全局入口页。

使用方法：
在终端中执行：
```bash
python3 gen_knowledge_graph.py --write-index
```
执行完毕后，最新生成的图谱块会自动嵌入到 [wiki/index.md](./wiki/index.md) 中，让你一眼看清知识点的拓扑关系。
